...
figma.ui.onmessage = async msg => {
  if (msg.type === 'generate-design') {
    ...
    await loadFont(data.manifest.headingFont || 'Inter');
    await loadFont(data.manifest.bodyFont || 'Inter');
    ...
    for (const sectionName of sectionsToCreate) {
      ...
      if (data.structuredPrompt && data.structuredPrompt.sections) {
         ...
         const matchedSection = data.structuredPrompt.sections.find((s:any) => s === sectionName || (s.type && s.type === sectionName));
